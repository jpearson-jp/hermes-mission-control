/* Mission Control — the owner's inbox: every decision parked on Jesse, oldest first. */
  ...
  function WaitingPage() {
    ...
    // The excluded bucket, straight off the payload — never recomputed. `recs`/`recoTotal` count the
    ...
    return h("div", { className: "mc-root" },
      ...
      h(Panel, {
        ...
        sub: framed.length + (q || tag ? " matching" : "") + " of " + framedTotal +
             " framed asks with options · " + recs.length + " carry a recommendation" + (excCount ? " · " + excCount + " excluded (" + excWhy + ")" : ""),
